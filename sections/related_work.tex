...
Both still assign each prompt a single, independently sampled binary verdict; \dataname's paired ladders hold the scenario fixed (same model and seed across all four rungs), letting us measure how a defense's suppression at one severity relates to another for a fixed scenario (the quantity Insight~3 requires); any flat prompt set cannot measure it.
