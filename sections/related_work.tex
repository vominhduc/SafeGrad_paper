...
Generation-side safety benchmarks all still assign each prompt a single, independently sampled binary verdict; \dataname's paired ladders instead hold the scenario fixed (same model and seed across all four rungs), exposing how a defense's suppression couples across severity---the quantity Insight~3 requires, which no flat prompt set can measure.
