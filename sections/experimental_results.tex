...
For example, MACE reduces NSFW L1 HGR from 25.4\% $\to$ 3.9\% ($\rho_{\mathrm{L1}} = 84.6\%$) and L3 from 73.8\% $\to$ 22.6\% ($\rho_{\mathrm{L3}} = 69.4\%$), raising the L3:L1 ratio from $2.9\times$ to $5.8\times$: \red{the strongest defense nearly closes the L1 gap on NSFW while residual L3 harm dominates.}
...
TRCE ($\Delta\mathrm{HGR} = 0.307$, SAS drop $= -0.008$) outperforms NP, SLD, Safree, UCE, SPEED, and SafetyDPO on both metrics at once\red{, without discounting the documented competitiveness of SLD or SafetyDPO in alternative settings}; RECE collapses on Legal and Social through concept diffuseness and semantic entanglement (Appendix~\ref{app:collapse}; e.g., MACE's erasure of Public Figures degrades benign-portrait SAS by 40.1\%).
...
All attacks exceed 55\% ASR: cross-modal methods lead (MMA-Diffusion 68.7\%, Ring-A-Bell 65.6\%, SneakyPrompt 63.4\%, UnlearnDiffAtk 61.5\%) over gradient-based P4D (55.6\%).
\red{We further evaluate the modern prefix-guided jailbreak (PGJ), a model-free black-box method fitting our protocol, against the best-defended configuration: ASR $=$ 72.4\% across intermediate rungs, showing that RL-based mutation is not required to exploit the gap. GenBreak-style attacks that surrogatize the target to exploit commercial black-box wrappers fall outside our weight-level diagnostic scope.}
