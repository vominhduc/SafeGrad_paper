...
A severity-aware filter fine-tuned on ladder data attains 95.2\% detection at L1 with a 3.8\% L0 false-positive rate, cutting the overall bypass rate from 88.0\% to 2.2\% and dominating all five baselines on every detection metric evaluated.
The result transfers to fresh generations from all six evaluated models (bypass rate at most 2.7\%), confirming that the diagnosed gap is trainable rather than intrinsic.
