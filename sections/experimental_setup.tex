...
\red{SD1.4 is selected for defense evaluation because (1) its U-Net architecture enables a controlled comparison across mitigation frameworks, since six of the nine defenses (UCE, RECE, SPEED, SafetyDPO, MACE, TRCE) are U-Net-specific weight-editing techniques with no published DiT implementation; and (2) it is the standard reference backbone in the concept-erasure literature. Defense effectiveness is largely architecture-independent, since it measures how well a mitigation objective suppresses harmful concepts; filters use static reference images (factoring T2I models out), and attack scores are averaged, so these findings are expected to transfer across backbones.}
...
\red{This per-prompt OR protocol models adversarial regeneration; all conclusions rest on cross-method comparisons under the identical protocol.}
\red{To quantify the role of deployment-time guard layers, we also evaluate two representative models (SD1.4, HiDream) with native safety checkers \emph{activated}: checkers suppress explicit L3 HGR to 47.3\% (SD1.4) and 63.4\% (HiDream), yet reduce L1/L2 HGR by at most 3.6\%, confirming that commercial checkers act largely as threshold switches for explicit content and leave the L1--L2 boundary unaddressed.}
...
\textbf{HGR} (harmful generation rate per severity level), \textbf{SBS} (normalized safety boundary slope), \textbf{SAS} (CLIP-based semantic alignment), \textbf{FBR} (filter bypass rate), and \textbf{ASR} (attack success rate) (Appendix~\ref{app:metrics})\red{; with 2{,}052 images per severity level and model, within-model Bernoulli standard errors on HGR stay at or below $1.1$ pp, and aggregated figures report $\pm 1$ SEM across the six models}.
