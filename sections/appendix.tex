...
\red{The severity-aware filter of Section~\ref{sec:filter} is trained as follows. Qwen2.5-VL-7B is fine-tuned with LoRA on the vision--language projection on (image, risk explanation) $\rightarrow$ YES/NO pairs, with level-balanced oversampling of L1--L2 samples. Ladders are split 80/10/10 with ladder integrity: all four rungs of a ladder remain in one split, preventing subject leakage between train and test. Evaluation metrics match Section~6.4 (per-level detection, FPR@L0, FBR, AUC, severity sensitivity); results appear as the bottom row of Table~\ref{tab:filter}, and the baselines are re-evaluated on the same held-out test split.}
