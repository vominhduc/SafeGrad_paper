...
\textbf{Cross-family replication.} A third run with InternVL2-8B---a different model family than every Qwen-based pipeline component---under the identical protocol (LoRA on the InternLM2 projections; 37.7M trainable parameters vs.\ the 7B's 47.6M) attains test-split FPR@L0 3.8\%, FBR 1.3\%, and L1 detection 98.1\%, versus a matched zero-shot anchor of FBR 40.1\% (L1 detection 32.7\%): a $\sim$31$\times$ reduction, confirming that the remediation recipe transfers across model families and is not a same-family teacher artifact.
