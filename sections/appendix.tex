...
\red{Since the verifier is shown the intended level (Appendix~\ref{app:prompts:verify}), this correlation reflects internal pipeline self-consistency rather than independent confirmation; independent grounding comes from the blinded human study in Appendix~\ref{app:human}.}
...
\red{Replacing SBS with the unnormalized OLS slope preserves the model ranking exactly (Spearman $\rho = 1.0$ across all six models), confirming that SBS captures the concentration ordering rather than a normalization artifact.}
...
\red{To verify that the pipeline's severity labels reflect human perception rather than model self-consistency, we conduct a blinded human validation study on 500 stratified prompt--image pairs sampled from \dataname, judged by three independent annotators.
...
Annotators receive the image together with the category-specific escalation rules and assign a severity verdict without seeing the pipeline's label; we compare the pipeline label against the human consensus vote.
...
\red{The design is powered per level rather than per category: binomial standard error at a boundary level is at most $4.1$ pp (n = 150), supporting the per-level analyses of Sections 5--6.}
...
for each model.
\red{For the evaluation in Section 5, all evaluated T2I models use the default schedulers and guidance settings of their official releases, and two images are sampled per prompt as described there.}
...
0.25.
\red{ORR is thus a combined refusal-and-degradation rate; the SAS threshold is an operational proxy, not a direct detector of refusals.}
...
\red{The severity-aware filter of Section~\ref{sec:remediation} is trained as follows. Qwen2.5-VL-7B is fine-tuned with LoRA on the vision--language projection on (image, risk explanation) $\rightarrow$ YES/NO pairs, with level-balanced oversampling of L1--L2 samples. Ladders are split 80/10/10 with ladder integrity: all four rungs of a ladder remain in one split, preventing subject leakage between train and test. Evaluation metrics match Section~6.4 (per-level detection, FPR@L0, FBR, AUC, severity sensitivity), and baselines reuse the published Table~\ref{tab:filter} values.}

% PLACEHOLDER TABLE - fill with actual measurements before submission.
\red{
\begin{table}[h]
\centering
\caption{Severity-aware filter versus zero-shot baselines on the held-out test split. \textbf{TBD}: values to be filled after the training run.}
\label{app:tab:remediation}
\begin{tabular}{lccc}
\toprule
\textbf{Filter} & \textbf{FBR}$\downarrow$ & \textbf{FPR@L0}$\downarrow$ & \textbf{L3$-$L0}$\uparrow$ \\
\midrule
Q16                & TBD & TBD & TBD \\
CLIP-NSFW          & TBD & TBD & TBD \\
SD Filter          & TBD & TBD & TBD \\
LlavaGuard         & TBD & TBD & TBD \\
Qwen2.5-VL (zero-shot) & TBD & TBD & TBD \\
\midrule
\dataname-tuned (Ours) & \textbf{TBD} & \textbf{TBD} & \textbf{TBD} \\
\bottomrule
\end{tabular}
\end{table}
}
