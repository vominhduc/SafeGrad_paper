...
\textbf{A2: Partially addressed; full cross-family run committed.}
Two clarifications and one commitment. First, the HGR \emph{ground truth} behind Insights 1--7 never comes from Qwen: the evaluator ensemble is InternVL2-8B, Phi-4-multimodal, and Gemma-3-4b-it (Sec.~4.2), all disjoint from the Qwen-based pipeline components. Qwen-family models appear only in construction scoring/verification and as the \emph{remediation backbone}. Second, within-backbone controls isolate the data effect from backbone priors: the tuned 7B improves over its own \emph{matched} zero-shot anchor (FBR 52.2\% $\to$ 2.2\%) and the identically-trained 3B over \emph{its own} anchor (FBR 37.5\% $\to$ 0.6\%; L1 detection 34.6\% $\to$ 99.0\%) --- gains of an order of magnitude at both model scales, which is difficult to reconcile with mere family-token artifacts. Third: we agree the remaining same-family gap (verifier Qwen3-VL $\to$ filter backbone Qwen2.5-VL) must be closed; we will train the identical recipe on InternVL2-8B and report FBR/FPR@L0 on the same ladder-respecting test split in the camera-ready. The three-MLLM ensemble labels provide a non-Qwen supervision target for exactly this purpose.
