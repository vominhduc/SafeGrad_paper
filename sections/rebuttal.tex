...
We additionally ran the second DiT check (commitment (b)): SLD-style token-level guided inference on CogView4-6B over the same test-split prompts (momentum-accumulated concept subtraction in the GLM conditioning space). Preliminary aggregation shows the same asymmetry --- relative suppression is stronger at L1 than at L3 ($\rho_{\mathrm{L1}} \approx 12.8\%$ vs.\ $\rho_{\mathrm{L3}} \approx 8.4\%$) --- and we will report the finalized numbers and per-rung HGR in the final manuscript should the vote aggregation shift them.
