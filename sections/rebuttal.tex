...
\textbf{W4: Western-centric taxonomy.} This is an inherited limitation from the safety-alignment data underlying current LLMs/MLLMs/T2I models, not a property of our dataset design, and was acknowledged in the supplement. We have moved the Appendix K finding into the main Discussion as a formal scope limitation: in the matched two-arm probe, Western-entity prompts are flagged *more* often (Public Figures 11.3\% vs 7.0\%; Copyright 31.3\% vs 25.7\%; directional, not significant at 5\%), so the severity calibration must be re-derived per jurisdiction --- we commit to the multilingual/multi-jurisdictional extension outlined there.
...
\textbf{W7: Western-centric taxonomy} As with the other reviewers, we have moved the Appendix K finding into the main Discussion as an explicit scope limitation (Western-entity prompts flagged more often: 11.3\% vs 7.0\% Public Figures; 31.3\% vs 25.7\% Copyright), highlighting that the severity descriptors reflect a US/EU regulatory baseline.
...
Sec.~5.3 (Insight~3) reports the NP defense on PixArt-$\alpha$ (a DiT): relative suppression is higher at L1 than at L3 on the same test split ($\rho_{\mathrm{L1}} = 9.5\%$ vs.\ $\rho_{\mathrm{L3}} = 6.0\%$). The full per-rung numbers are: undefended vs.\ NP HGR of 20.2\% $\to$ 18.3\% (L1), 63.5\% $\to$ 58.7\% (L2), 80.8\% $\to$ 76.0\% (L3) ($n = 104$ per rung, held-out split) --- the relative-suppression asymmetry survives on a DiT backbone, at NP's characteristically mild magnitude. These per-rung numbers are now in the main text (commitment (a) delivered).
We additionally executed the second DiT check (commitment (b)): SLD-style token-level guided inference on CogView4-6B over the same test-split prompts (momentum-accumulated concept subtraction in the GLM conditioning space; edit application verified, 50/50 conditional edits per generation). The measured result is that this direct port is near-inert: $\rho_{\mathrm{L1}} = 0.0\%$ (HGR 22.1\% $\to$ 22.1\%) versus $\rho_{\mathrm{L3}} = 1.1\%$ (HGR 86.5\% $\to$ 85.6\%), $n = 104$/rung. We report this honestly: negative prompting transfers the Insight-3 asymmetry to a DiT (PixArt), but token-threshold guidance ports do not transfer trivially to DiT text encoders and require architecture-specific retuning, consistent with the paper's observation that defenses are architecture-bound. The main text now states both outcomes.
Weight-editing and fine-tuning defenses remain SD1.4-only for the reasons in Sec.~4.1: six of the nine are U-Net-specific editors with no public DiT implementation.
