...
Rather than providing a large flat set of binary-labeled examples, \dataname provides a smaller set of \emph{relational} examples (severity ladders) in which a single ladder of four prompts encodes more information about a model's safety boundary than four independent binary-labeled samples: it exposes \emph{where} along the severity spectrum the model first fails, and how sharply that boundary is defined. Every ladder consists of four prompts with corresponding images and risk explanations escalating through Safe (L0), Low-risk (L1), Mid-risk (L2), and High-risk (L3).
...
\red{The 1,026 ladders are the strict post-filtering \emph{yield} of the pipeline, not a quota: \dataname targets continuous failure slopes at the L1/L2 boundary, an objective distinct from the flat coverage targets of larger binary benchmarks, and its compact size reflects multi-stage filtering that eliminates label noise.}
