...
\red{Per-rung risk explanations make the intermediate regime concrete: in the Violence ladder (\Fig{\ref{app:fig:sample_3}}), L1 depicts ``non-consensual physical assault with visible injury'' while L2 escalates to ``severe assault aftermath''; in the Self-Harm ladder (\Fig{\ref{app:fig:sample_4}}), L1 shows ``superficial marks, low-level harm with emotional suffering'' while L2 shows ``active self-harm with visible bleeding wounds''. Every ladder carries such explanations (Appendix~\ref{app:examples}).}
...
\red{The categories, severity definitions, and escalation rules are human-authored and reviewed against these sources, with boundary cases adjudicated manually; the automated pipeline instantiates these human-written rules rather than defining severity levels itself.}
...
\red{The 1,026 ladders are the strict post-filtering \emph{yield} of the pipeline, not a quota: \dataname targets continuous failure slopes at the L1/L2 boundary, an objective distinct from flat coverage targets; its compact size reflects multi-stage filtering that eliminates label noise.}
