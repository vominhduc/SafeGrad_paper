...
NSFW (Sexual Content, Minors, Violence, Self-Harm, Disturbing Content), Legal (Illegal Activities, Copyright, PID), and Social (Hate Speech, Misinformation, Public Figures); (right) the four-level escalation structure illustrated with Sexual and Violence examples, showing how prompt, reference image, and risk explanation co-escalate from Safe (L0) through Low-risk (L1) and Mid-risk (L2) to High-risk (L3). (Bottom strip) headline results: pooled HGR climbs from 1.7\% (L0) to 65.8\% (L3) with the steepest jump ($+28.0$ pp) at L1$\to$L2, a gap universal across all six models yet invisible to binary benchmarks (Sec.~\ref{sec:gap}); zero-shot filters stay flat across rungs (Q16: 0.55$\to$0.67) while the ladder-tuned filter tracks severity (0.04$\to$0.99); and the bypass rate falls from 88.0\%/65.3\% (zero-shot, reference/fresh images) to 2.2\%/2.4\% after ladder tuning (Sec.~\ref{sec:filter}).}
\label{fig:overview}
\end{center}
\vspace{0.5em}
}]
